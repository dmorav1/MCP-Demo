...
async def test_create_conversation(crud_instance, mock_db_session):
    ...
    result = await crud_instance.create_conversation(conversation_data)
...
@pytest.mark.asyncio
async def test_search_conversations(crud_instance, mock_db_session, mock_embedding_service):
