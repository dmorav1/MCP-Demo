...
@app.post(
    "/ingest", response_model=schemas.Conversation, status_code=status.HTTP_201_CREATED
)
async def ingest_conversation(
    conversation_data: schemas.ConversationIngest,
    conversation_crud: crud.ConversationCRUD = Depends(
        dependencies.get_conversation_crud
    ),
):
    """
...
async def get_conversations(
    ...
    limit: int = Query(
        100, ge=1, le=1000, description="Maximum number of conversations to return"
    ),
    conversation_crud: crud.ConversationCRUD = Depends(
        dependencies.get_conversation_crud
    ),
