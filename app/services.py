...
class EmbeddingService:
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Returning zero vector fallback with dimension: {len(fallback_vector)}"
            )
...
class ConversationProcessor:
    ...
    def chunk_conversation(
        self, messages: List[Dict[str, Any]], max_chunk_size: int = 1000
    ) -> List[Dict[str, Any]]:
        """
        Chunk conversation messages into smaller pieces suitable for embedding
        """
        logger.info(
            f"📝 Starting to chunk {len(messages)} messages with max_chunk_size: {max_chunk_size}"
        )
        chunks = []
        current_chunk = ""
        current_messages = []
        ...
        for i, message in enumerate(messages):
            message_text = (
                f"{message.get('author_name', 'Unknown')}: {message.get('content', '')}"
            )
            ...
            if len(current_chunk + message_text) > max_chunk_size and current_chunk:
                logger.debug(
                    f"🔄 Creating chunk {len(chunks)} with {len(current_messages)} messages"
                )
                chunks.append(
                    {
        ...
        if current_chunk.strip():
            logger.debug(
                f"🔄 Creating final chunk {len(chunks)} with {len(current_messages)} messages"
            )
            chunks.append(
                {
                    "order_index": len(chunks),
    ...
    async def process_conversation_for_ingestion(
        ...
        """
        logger.info(
            f"🚀 Starting conversation processing for: {conversation_data.get('scenario_title', 'Unknown scenario')}"
        )
        ...
        logger.info(
            f"✅ Conversation processing completed: {len(chunks)} chunks with embeddings"
        )
        return result
